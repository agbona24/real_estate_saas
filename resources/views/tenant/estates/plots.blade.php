...
function showPlotDetails(plotNumber, status, client) {
    ...
    let content = `
        <div class="space-y-6">
            ...
            <div class="flex space-x-3">
                ${status === 'available' ? `
                    <button onclick="window.location.href='{{ route('tenant.estates.allocate', $estate->id ?? 1) }}?plot=${plotNumber}'" class="flex-1 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-md font-semibold">
